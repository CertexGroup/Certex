...
Router.route('/create_certificate', function () {
  console.log(Authorities.findOne({name: "ITESM"}));
  this.render('createCertificate', {
    data: {
      authority: Authorities.find({name: "ITESM MTY"})
    }
  });
});
...
Router.route('/register_authority', function(){
  this.render('registerAuthority');
});
Router.route('/register_individual',function(){
  this.render('registerIndividual');
});
